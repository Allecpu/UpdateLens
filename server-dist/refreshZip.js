...
const runRefreshScripts = async (sources) => {
...
export const buildRefreshZip = async (sources) => {
    ...
    if (microsoftFile && microsoftPayload) {
        const msRaw = await readFile(path.resolve(repoRoot, 'public', 'data', microsoftFile), 'utf-8');
        dataFolder.file(microsoftFile, msRaw);
    }
    if (eosFile && eosPayload) {
        const eosRaw = await readFile(path.resolve(repoRoot, 'public', 'data', eosFile), 'utf-8');
