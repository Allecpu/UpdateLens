...
const appendFollowUpSuggestions = (response) => {
    const query = typeof response.filterPatch.query === 'string' && response.filterPatch.query.trim().length > 0
        ? response.filterPatch.query.trim()
        : null;
    const sourceLabel = Array.isArray(response.filterPatch.sources) && response.filterPatch.sources.length > 0
