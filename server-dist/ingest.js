...
const buildSnapshotKey = (releasePlanId, featureName) => {
...
const normalizeItem = (raw, language, snapshotIndex) => {
...
const run = async () => {
    ...
    const lastSnapshot = db
        .prepare(`SELECT etag, hash FROM release_plan_snapshots WHERE language = ? ORDER BY snapshot_id DESC LIMIT 1`)
        .get(LANGUAGE);
    ...
    const snapshotStmt = db.prepare(`INSERT INTO release_plan_snapshots (fetched_at, language, etag, hash, raw_payload, schema_version)
     VALUES (?, ?, ?, ?, ?, ?)`);
    const snapshotInfo = snapshotStmt.run(now, LANGUAGE, etag, hash, text, SCHEMA_VERSION);
    const snapshotId = Number(snapshotInfo.lastInsertRowid);
    const selectItem = db.prepare(`SELECT * FROM release_plan_items WHERE source_plan_id = ? AND language = ?`);
    const upsertItem = db.prepare(`INSERT INTO release_plan_items (
    ...
    const insertHistory = db.prepare(`INSERT INTO release_plan_history (item_id, snapshot_id, change_type, diff, changed_at)
     VALUES (?, ?, ?, ?, ?)`);
    const deleteAvailability = db.prepare(`DELETE FROM release_plan_availability_types WHERE item_id = ?`);
    const insertAvailability = db.prepare(`INSERT INTO release_plan_availability_types (item_id, type) VALUES (?, ?)`);
    let inserted = 0;
    let updated = 0;
    const transaction = db.transaction((items) => {
        items.forEach((raw) => {
            ...
            const existing = selectItem.get(normalized.sourcePlanId, LANGUAGE);
            const nowLocal = new Date().toISOString();
            const createdAt = existing?.created_at ? String(existing.created_at) : nowLocal;
            const info = upsertItem.run(normalized.sourcePlanId, normalized.releasePlanId, normalized.learnUrl, normalized.appName, normalized.productName, normalized.featureName, normalized.summary, normalized.investmentArea, normalized.status, normalized.wave, normalized.availabilityDate, normalized.availabilityDateFull, normalized.firstAvailableDate, normalized.lastUpdatedDate, normalized.enabledFor, normalized.geographyHtml, normalized.language, normalized.sourceUrl, createdAt, nowLocal);
            const itemId = Number(info.lastInsertRowid);
            if (!existing) {
                inserted += 1;
                insertHistory.run(itemId, snapshotId, 'insert', JSON.stringify({}), nowLocal);
            }
            else {
                ...
                if (Object.keys(diff).length > 0) {
                    updated += 1;
                    insertHistory.run(Number(existing.id), snapshotId, 'update', JSON.stringify(diff), nowLocal);
                }
            }
    ...
    console.log(`[Ingest] Snapshot ${snapshotId} stored. inserted=${inserted} updated=${updated}`);
};
run().catch((error) => {
