...
export interface FilterPresetShareRow {
  share_id: string;
  preset_id: string;
  grantee_tenant_id: string | null;
  grantee_object_id: string | null;
 ...
 */
const rowToResponse = (row: FilterPresetRow, user: UserIdentity): PresetResponse => {
  return {
    id: row.preset_id,
    name: row.name,
 ...
 */
export const canModifyPreset = (user: UserIdentity, preset: FilterPresetRow): boolean => {
 ...
 */
export const getPresetsForUser = (
  db: Database.Database,
  user: UserIdentity,
  ...
  `).all({ tid: user.tid, oid: user.oid }) as FilterPresetRow[];
  ...
  const sharedPresets = db.prepare(`
    ...
    ORDER BY p.name ASC
  `).all({ tid: user.tid, oid: user.oid, email: user.email }) as FilterPresetRow[];
...
export const getPresetById = (
  db: Database.Database,
  presetId: string
): FilterPresetRow | undefined => {
  return db.prepare(`
    SELECT * FROM filter_presets WHERE preset_id = @presetId
  `).get({ presetId }) as FilterPresetRow | undefined;
};
...
export const updatePreset = (
  ...
  const params: Record<string, unknown> = { presetId, now };
 ...
 * Delete a preset
 */
export const deletePreset = (db: Database.Database, presetId: string): void => {
...
export const getSharingInfo = (
  ...
  `).all({ presetId }) as Array<{ grantee_email: string; permission: string; created_at: string }>;
 ...
 */
export const getOutgoingShares = (
  db: Database.Database,
  user: UserIdentity
): Array<PresetResponse & { sharedWith: ShareInfo[] }> => {
  ...
  return presets.map(preset => {
    const shares = db.prepare(`
      ...
      FROM filter_preset_shares
      WHERE preset_id = @presetId
    `).all({ presetId: preset.preset_id }) as Array<{ grantee_email: string; permission: string; created_at: string }>;
 ...
 */
export const getIncomingShares = (
  db: Database.Database,
  user: UserIdentity
): Array<PresetResponse & { sharedBy: { email: string; name: string | null } }> => {
  const presets = db.prepare(`
    SELECT DISTINCT p.* FROM filter_presets p
    LEFT JOIN filter_preset_shares s ON p.preset_id = s.preset_id
    WHERE (p.owner_tenant_id != @tid OR p.owner_object_id != @oid)
...
/**
 * Migrate presets from localStorage format
 */
export interface LocalStoragePreset {
  id: string;
  name: string;
  description?: string;
  filters: unknown;
...
export const migratePresetsFromLocalStorage = (
  db: Database.Database,
  user: UserIdentity,
  presets: LocalStoragePreset[]
