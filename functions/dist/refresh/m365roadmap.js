...
import { todayStamp, monthStamp, slugify, normalizeText, isValidHttpUrl } from '../shared/utils.js';
...
async function fetchWithRetry(url, retries = MAX_RETRIES) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            console.warn(`[RefreshM365Roadmap] Attempt ${attempt}/${retries} failed:`, error instanceof Error ? error.message : error);
            if (attempt === retries)
                throw error;
...
async function fetchAllM365Data() {
...
function buildAvailabilityTypes(raw) {
    const types = [];
 ...
 */
const buildReleaseItem = (raw, productName) => {
    ...
    rawItems.forEach((raw) => {
        try {
            ...
            if (products.length === 0) {
                console.warn(`[RefreshM365Roadmap] No products found for item ${raw.id}, using fallback "Microsoft 365"`);
            ...
            console.warn(`[RefreshM365Roadmap] Skipped invalid item ${raw.id}:`, error instanceof Error ? error.message : error);
            skippedIds.push(String(raw.id));
        }
    });
    if (skippedIds.length > 0) {
        console.warn(`[RefreshM365Roadmap] Skipped ${skippedIds.length} invalid items:`, skippedIds.slice(0, 10));
    }
    console.log(`[RefreshM365Roadmap] Transformed ${rawItems.length} raw items into ${validItems.length} release items`);
    return validItems;
}
export const refreshM365Roadmap = async () => {
