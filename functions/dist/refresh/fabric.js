...
import { todayStamp, monthStamp, normalizeText } from '../shared/utils.js';
...
const parseFabricDateFull = (dateStr) => {
...
async function fetchWithRetry(url, retries = MAX_RETRIES) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            console.warn(`[RefreshFabric] Attempt ${attempt}/${retries} failed:`, error instanceof Error ? error.message : error);
            if (attempt === retries) {
                throw error;
            }
...
function buildStatus(raw) {
    ...
    if (raw.release_status === 'Planned')
        return 'Planned';
    console.warn(`[RefreshFabric] Unknown status combination for ${raw.release_item_id}:`, { status: raw.release_status, type: raw.release_type });
    return 'Unknown';
}
...
function extractItems(rawItems) {
    ...
    rawItems.forEach((raw) => {
        try {
            ...
            console.warn(`[RefreshFabric] Skipped invalid item ${raw.release_item_id}:`, error instanceof Error ? error.message : error);
            skippedIds.push(raw.release_item_id);
        }
    });
    if (skippedIds.length > 0) {
        console.warn(`[RefreshFabric] Skipped ${skippedIds.length} invalid items:`, skippedIds.slice(0, 10));
    }
    return validItems;
}
export const refreshFabric = async () => {
