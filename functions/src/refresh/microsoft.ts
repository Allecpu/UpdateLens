...
import type { MicrosoftReleaseItem, Snapshot, RefreshResult } from '../shared/types.js';
...
const buildSnapshotKey = (releasePlanId: string, featureName: string): string =>
  `${releasePlanId}::${normalizeText(featureName).toLowerCase()}`;
...
const normalizeDocUrl = (value?: string | null): string | null => {
  ...
  if (trimmed.startsWith('/')) return `https://learn.microsoft.com${trimmed}`;
  return null;
};
...
const fetchSnapshotIndex = async (): Promise<Map<string, { snapshotId: string; learnUrl: string | null }>> => {
  ...
  while (nextUrl) {
    ...
    payload.value.forEach((row) => {
      if (!row._mssh_releaseplan_value || !row.mssh_featurename) return;
...
const buildStatus = (raw: RawMicrosoftItem): MicrosoftReleaseItem['status'] => {
  if (raw['GA date']) return 'Launched';
  if (raw['Public preview date']) return 'Try now';
  if (raw['Early access date']) return 'Planned';
  return 'Unknown';
};

const buildAvailabilityTypes = (raw: RawMicrosoftItem): string[] => {
  const types: string[] = [];
  if (raw['Early access date']) types.push('Early Access');
  if (raw['Public preview date']) types.push('Public Preview');
  if (raw['GA date']) types.push('GA');
  return types;
};

const extractCountriesFromHtmlNode = (html: string): string[] => {
  if (!html) return [];
...
const extractItems = (
  ...
  const items: MicrosoftReleaseItem[] = [];
...
export const refreshMicrosoft = async (): Promise<RefreshResult> => {
  ...
  try {
    ...
    const snapshot: Snapshot<MicrosoftReleaseItem> = {
      version: 1,
      items
    };
